...
function Tab({ tab, color, isActive, layoutAnimation = _layout, tintColor = "#000", tabStyle, textProps, expoVectorIconProps, showCloseIcon, showTexts, customCloseIcon, textAnimation, onPress, }) {
    return (<AnimatedPressable style={[styles.tabContainer, { backgroundColor: color }, tabStyle]} key={`${tab.name}-${tab.icon}`} onPress={() => onPress(tab)} layout={layoutAnimation}>
